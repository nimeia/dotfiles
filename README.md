...
推荐从 Ubuntu Desktop 的默认安装开始。Server 版或最小安装也能用，但需要补齐更多桌面组件，
出现缺 portal、keyring、polkit、XWayland 集成等问题的概率更高。
...
软链接和系统登录模板，只运行缺失的阶段。Ubuntu Desktop 测试建议先查看 Desktop profile 计划：
...
只想先跳过 Emacs 时，`--skip-doom` 会同时跳过 Doom/Emacs apt 包和 npm language server：

```bash
./bootstrap.sh --skip-doom
```

需要通过代理拉 GitHub 或 npm 代码时，可以临时设置 `HTTP_PROXY`/`HTTPS_PROXY`/`ALL_PROXY`。
apt 默认不继承这些通用代理；只有明确设置 `DOTFILES_APT_PROXY=http://...` 时 apt 才会走代理。
npm 会继承通用代理，也可以用 `DOTFILES_NPM_PROXY=http://...` 或 `DOTFILES_NPM_REGISTRY=...`
单独指定 npm 代理或 registry。
...
```bash
./install.sh --profile desktop --packages
```

这一步是后续所有步骤的基础。它会安装 niri/Wayland 运行时依赖、开发工具、fcitx5、Waybar、
portal 后端、Rust/cargo，并从源码安装 `xwayland-satellite`，用于 niri 下运行 Warp Terminal 等
仍依赖 X11 后端的程序。Doom/Emacs 支持包只在未使用 `--skip-doom` 或未传
`--skip-doom-packages` 时安装。`minimal` profile 会额外安装 greetd、PipeWire 和 Bluetooth
基础服务；`desktop` profile 不会重复接管这些系统服务。

2. 安装外部包和非 apt 项：

```bash
./install.sh --profile desktop --external
```

这一步依赖 `--packages` 里的 `curl`、`jq`、`git`、`cargo` 等工具。它会安装 Chrome、Yazi、
swww/swww-daemon、Nerd Font Symbols、轻量壁纸集合，以及 apt 中可用的 Ghostty。`desktop` profile
下不会自动把 Chrome 设为默认浏览器；需要接管默认浏览器时可运行
`DOTFILES_SET_DEFAULT_BROWSER=1 ./install.sh --profile desktop --external`。
...
```bash
./install.sh --profile desktop --wallpapers
```

默认会把 Catppuccin 壁纸集合中的 `gradients` 和 `minimalistic` 目录浅克隆到
`~/Pictures/Wallpapers/catppuccin-wallpapers`，`wallpaper-random` 会递归扫描
`~/Pictures/Wallpapers`。需要更多目录时可设置 `WALLPAPER_SPARSE_PATHS`。
...
它会依次执行 packages、external、用户配置、profile 对应的系统模板和 Doom 配置。niri 源码构建
仍然单独使用 `./install.sh --niri-source`，因为它可能需要按机器选择 ref。新机器首次安装时不要
直接用 `--all` 替代完整顺序，除非已经先安装好了 niri。
